...
> The system preference 'OverdueNoticeBcc' seems, by its name, to imply that a BCC will be added to all overdue notices. The description says that the address will be added to all notices. The behavior of the system follows the description, not the system preference name. The system preference is re-named to NoticeBcc.
...
> This patch was only ever intended to 'add new' user record from Shibboleth into Koha ('provision') and not to 'sync' Shibboleth with an existing Koha user. It is possible to map Sbibboleth's attributes with Koha fields, the configuration is done in koha-conf.xml
...
- [[18461]](http://bugs.koha-community.org/bugzilla3/show_bug.cgi?id=18461) Internal server error while approving OPAC-edited patron attributes containing umlauts (���)
...
Autogenerated release notes updated last on 11 May 2017 09:03:19.
